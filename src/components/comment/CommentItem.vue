      ...
      <div class="me-reply-list">
        <div class="me-reply-item" v-for="c in comment.childrens" :key="c.id">
          <div style="font-size: 14px">
            <img class="me-view-picture" style="width: 30px; height: 30px;" :src="c.author.avatar"></img>
            <span class="me-reply-user">{{c.author.nickname}}:&nbsp;&nbsp;</span>

            <span v-if="c.level == 2" class="me-reply-user">@{{c.toUser.nickname}}  </span>

            <span>{{c.content}}</span>
            <a class="me-view-comment-tool" style="padding-left: 10px;" @click="showComment(c.id, c.author)">
              <i class="me-icon-comment"></i>&nbsp;回复
            </a>
          </div>
          <div class="me-view-meta">
            <span style="padding-right: 20px;">{{c.createDate | format}}</span>
          </div>
      ...
      publishComment() {
        ...
        publishComment(that.reply,this.$store.state.token).then(data => {
          if(data.success){
            ...
            this.showComment(this.commentShowIndex+1)
          }else{
